...
Deno.serve(async (req) => {
  try {
    // 1. Create services_registry table
    ...
    // 2. Insert services into registry
    const services = [
      {
        service_key: 'stripe',
      ...
      {
        service_key: 'datev',
        display_name: 'DATEV Export',
        description: 'Accounting software export',
        integration_type: 'supabase_edge',
        edge_function_name: 'datev-export',
        is_active: true,
        apps_enabled: ['vermietify', 'fintutto'],
        pricing: { per_export: 5.00 },
        cost_per_call: 1.50,
      },
    ];
    ...
    return Response.json({
      success: true,
      message: 'Services registry initialized',
