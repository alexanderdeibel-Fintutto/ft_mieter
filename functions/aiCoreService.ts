...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!budgetCheck.allowed) {
      // Budget-Status automatisch auf "budget_exceeded" setzen
      if (settings.id) {
      ...
      // Alle Features deaktivieren bei Budget-Überschreitung
      ...
      return Response.json({ 
        success: false, 
        error: "Monatliches AI-Budget erreicht - alle AI-Features deaktiviert",
    ...
    // 3. Rate-Limit prüfen
    const rateLimitCheck = await checkRateLimit(base44, userEmail, settings, featureKey);
