-- ================================================
-- FINTUTTO COMPLETE SETUP (v2 - Clean Install)
-- Dieses Script in den Supabase SQL Editor kopieren
-- und einmal ausfuehren.
--
-- ACHTUNG: Loescht und erstellt alle Tabellen neu!
-- (Nur Revenue/Affiliate Tabellen, nicht eure App-Daten)
-- ================================================

-- ================================================
-- TEIL 0: Alte Tabellen loeschen (falls vorhanden)
...
DROP TABLE IF EXISTS ab_test_configs CASCADE;
DROP TABLE IF EXISTS ecosystem_bundle_config CASCADE;
DROP TABLE IF EXISTS white_label_unit_tenants CASCADE;
DROP TABLE IF EXISTS white_label_units CASCADE;
DROP TABLE IF EXISTS white_label_tenants CASCADE;
DROP TABLE IF EXISTS transaction_usage CASCADE;
DROP TABLE IF EXISTS ecosystem_bundle_events CASCADE;
DROP TABLE IF EXISTS ecosystem_cross_sell_log CASCADE;
DROP TABLE IF EXISTS affiliate_user_preferences CASCADE;
DROP TABLE IF EXISTS affiliate_tracking CASCADE;
...
CREATE TABLE affiliate_user_preferences (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE ecosystem_cross_sell_log (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id TEXT,
...
CREATE TABLE ecosystem_bundle_events (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id TEXT NOT NULL,
...
-- 8. White-Label Units
CREATE TABLE white_label_units (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE white_label_unit_tenants (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id UUID NOT NULL REFERENCES white_label_tenants(id) ON DELETE CASCADE,
...
CREATE TABLE ecosystem_bundle_config (
  id TEXT PRIMARY KEY,
...
CREATE INDEX idx_affiliate_tracking_user ON affiliate_tracking(user_id);
CREATE INDEX idx_affiliate_tracking_provider ON affiliate_tracking(provider);
CREATE INDEX idx_affiliate_tracking_created ON affiliate_tracking(created_at);
CREATE INDEX idx_affiliate_tracking_event ON affiliate_tracking(event_type);
CREATE INDEX idx_affiliate_prefs_user ON affiliate_user_preferences(user_id);
CREATE INDEX idx_crosssell_log_user ON ecosystem_cross_sell_log(user_id);
CREATE INDEX idx_crosssell_log_source ON ecosystem_cross_sell_log(source_app);
CREATE INDEX idx_crosssell_log_created ON ecosystem_cross_sell_log(created_at);
CREATE INDEX idx_bundle_events_user ON ecosystem_bundle_events(user_id);
CREATE INDEX idx_bundle_events_bundle ON ecosystem_bundle_events(bundle_id);
CREATE INDEX idx_tx_usage_user ON transaction_usage(user_id);
CREATE INDEX idx_tx_usage_type ON transaction_usage(transaction_type);
CREATE INDEX idx_tx_usage_created ON transaction_usage(created_at);
CREATE INDEX idx_wl_tenants_owner ON white_label_tenants(owner_user_id);
CREATE INDEX idx_wl_tenants_status ON white_label_tenants(status);
CREATE INDEX idx_wl_units_tenant ON white_label_units(tenant_id);
CREATE INDEX idx_affiliate_partners_active ON affiliate_partners(is_active, priority DESC);
CREATE INDEX idx_affiliate_partners_category ON affiliate_partners(category);
...
-- Service role: full access (fuer Edge Functions)
CREATE POLICY "service_full" ON affiliate_partners FOR ALL TO service_role USING (true) WITH CHECK (true);
CREATE POLICY "service_full" ON affiliate_tracking FOR ALL TO service_role USING (true) WITH CHECK (true);
CREATE POLICY "service_full" ON affiliate_user_preferences FOR ALL TO service_role USING (true) WITH CHECK (true);
CREATE POLICY "service_full" ON ecosystem_cross_sell_log FOR ALL TO service_role USING (true) WITH CHECK (true);
CREATE POLICY "service_full" ON ecosystem_bundle_events FOR ALL TO service_role USING (true) WITH CHECK (true);
CREATE POLICY "service_full" ON transaction_usage FOR ALL TO service_role USING (true) WITH CHECK (true);
CREATE POLICY "service_full" ON white_label_tenants FOR ALL TO service_role USING (true) WITH CHECK (true);
CREATE POLICY "service_full" ON ecosystem_bundle_config FOR ALL TO service_role USING (true) WITH CHECK (true);
CREATE POLICY "service_full" ON ab_test_configs FOR ALL TO service_role USING (true) WITH CHECK (true);
CREATE POLICY "service_full" ON ab_test_assignments FOR ALL TO service_role USING (true) WITH CHECK (true);
CREATE POLICY "service_full" ON ab_test_events FOR ALL TO service_role USING (true) WITH CHECK (true);

-- Anon: INSERT erlaubt (fuer Client-seitiges Tracking)
CREATE POLICY "anon_insert" ON affiliate_tracking FOR INSERT TO anon WITH CHECK (true);
CREATE POLICY "anon_insert" ON ecosystem_cross_sell_log FOR INSERT TO anon WITH CHECK (true);
CREATE POLICY "anon_insert" ON ecosystem_bundle_events FOR INSERT TO anon WITH CHECK (true);
CREATE POLICY "anon_insert" ON ab_test_events FOR INSERT TO anon WITH CHECK (true);

-- Anon: SELECT fuer Konfig-Tabellen (werden im Frontend gelesen)
CREATE POLICY "anon_select" ON affiliate_partners FOR SELECT TO anon USING (true);
CREATE POLICY "anon_select" ON ab_test_configs FOR SELECT TO anon USING (true);
CREATE POLICY "anon_select" ON ecosystem_bundle_config FOR SELECT TO anon USING (true);
...
-- ================================================
-- TEIL 6: Seed Data - Bundle Configs
-- ================================================
...
-- ================================================
-- FERTIG! Verifizierung:
-- ================================================

SELECT 'Erstellt: ' || count(*) || ' Tabellen' as status
